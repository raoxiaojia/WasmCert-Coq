...
let app_vunop_str op v =
  let vw = of_bits v in
  let wasm_f = 
    match Extracted_z.int_of_z op with
    | 77 -> V1x128.lognot
    | 96 -> I8x16.abs
    | 97 -> I8x16.neg
    | 98 -> I8x16.popcnt
...
let app_vbinop_str op_args v1 v2 = 
  ...
  let iop = Extracted_z.int_of_z op in
  let iargs = List.map Extracted_z.int_of_z args in
...
let app_vtestop_str op v1 =
  let v1w = of_bits v1 in
  let op_i = Extracted_z.int_of_z op in
  match op_i with
